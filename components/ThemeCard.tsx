...
export default function ThemeCard({ theme, onClick }: ThemeCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="group relative flex w-full cursor-pointer flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all hover:shadow-md sm:flex-row sm:h-48"
        >
            {/* Image Section */}
            <div className="relative h-48 w-full shrink-0 overflow-hidden bg-gray-100 sm:h-full sm:w-40">
                <Image
                    src={theme.posterUrl}
                    alt={theme.name}
                    fill
